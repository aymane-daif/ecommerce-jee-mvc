...
public class ConnDb {
    ...
    public ConnDb(String url, String driver, String username, String password){
        try{
        	...
        	String clientSt = "CREATE TABLE IF NOT EXISTS client (id INT NOT NULL AUTO_INCREMENT,email VARCHAR(70) UNIQUE, nom VARCHAR(100), prenom VARCHAR(100), tel VARCHAR(20), adresse VARCHAR(70), ville VARCHAR(20), codePostal INT, motDePasse VARCHAR(255), CONSTRAINT pk_client PRIMARY KEY(id));";
        	...
        	String articleSt = "CREATE TABLE IF NOT EXISTS article (codeArticle INT NOT NULL AUTO_INCREMENT, designation VARCHAR(255), prix double, stock INT, photo BLOB, refCat INT, CONSTRAINT pk_article PRIMARY KEY(codeArticle), CONSTRAINT fk_categorie FOREIGN KEY(refCat) REFERENCES categorie(refCat));";
